 ...
 * @author      Ben
 * @date        2021/07/26
 * @description Stackable Planetary Gearbox for 775 DC Motor
 */
...
// part can be sun, intcarrier, finalcarrier, planet, housing or cover
module print_parts(ring_teeth=32, sun_teeth=8, stages=2, part="housing") {
    ...
    if (part=="housing") {
        housing(housing_teeth=ring_teeth, stages=stages);
    } else if (part=="motor") {
        motorMount(stages=stages);
    } else if (part=="sun") {
        sun(teeth=sun_teeth);
    } else if (part=="planet") {
...
module planet(teeth=12, pitch=0.865, gear_thickness=11, pressure_angle=pressure_angle, bearing_diam=7.4, bearing_thickness=2.75, bearing_spacing=2, bore=3, vertical_space=1.0) {
    intersection () {
        difference() {
                // outer gear
                gear(number_of_teeth=teeth, diametral_pitch=pitch, hub_diameter=0, hub_thickness=gear_thickness-vertical_space, bore_diameter=bore,
                     rim_thickness=gear_thickness-vertical_space, rim_width=0, gear_thickness=gear_thickness-vertical_space, clearance=clearance,
                     backlash=backlash, pressure_angle=pressure_angle, $fn=10);
                // // upper space
...
module test_ring(motor_size=42, housing_teeth=32, pitch=0.865, pressure_angle=pressure_angle, wall=1, gear_thickness=11) {
    ...
    difference() {
        ...
        gear(number_of_teeth=housing_teeth, diametral_pitch=pitch, hub_diameter=0, bore_diameter=0, rim_thickness=total_height, gear_thickness=total_height,
             clearance=0, backlash=-backlash, pressure_angle=pressure_angle, addendum_adjustment=addendum_adjustment);
    }
}
/**/

/**
 * Piece in between dc motor and housing, where needed
 */
module motorMount(motor_size=42,  stages=1, gear_thickness=11, totalHeight=12, carrier_thickness=6, bolt_spacing=33, bolt_size=3.5) {
    ...
    translate([0,0,total_height/2]) difference() {
        ...
        translate([15, 0, 0]) {
            cylinder(r=3, h=6, $fn=6);      // Hexagon
            cylinder(r=bolt_size/2, h=total_height*2+extra, center=true, $fs=circle_precision);
        }
        translate([-15, 0, 0]) {
            cylinder(r=3, h=6, $fn=6);      // Hexagon
            cylinder(r=bolt_size/2, h=total_height*2+extra, center=true, $fs=circle_precision);
        }
        // central motor shaft rod
        translate([0, 0, -totalHeight+5]) {
            cylinder(d=22, h=5, $fs=circle_precision);
            cylinder(d=7, h=total_height*3+extra, center=true, $fs=circle_precision);
        }
    }
}
/**/
 ...
 * housing piece
 */
module housing(motor_size=42, housing_teeth=32, pitch=0.865, pressure_angle=pressure_angle, wall=1, recess_depth=2, stages=1, gear_thickness=11, 
               gear_height=12, pilot_diam=23, pilot_length=2, carrier_thickness=6, bolt_spacing=33, bolt_size=3.5) {
    ...
    translate([0,0,total_height/2]) difference() {
        ...
        translate([0,0,-total_height/2-extra/2])
                  cylinder(r=pilot_diam/2-2.5,h=pilot_length, $fs=circle_precision);
        // inner funnel [r1=pilot_diam/2]
        translate([0,0,-total_height/2-extra+pilot_length])
                  cylinder(r1=gear_diam/2-1, r2=gear_diam/2-2.5, h=gear_height+extra*2-pilot_length, $fs=circle_precision);
        // top hole
        translate([0,0, total_height/2-recess_depth/2-carrier_thickness/2])
                  cylinder(r=motor_size/2-wall,h=carrier_thickness+recess_depth+extra, center=true, $fs=circle_precision);
        // inner gear
        translate([0,0,gear_height-total_height/2]) 
                  gear(number_of_teeth=housing_teeth, diametral_pitch=pitch, hub_diameter=0, bore_diameter=0, rim_thickness=ring_height+extra,
                                              gear_thickness=ring_height+extra,clearance=0, backlash=-backlash, pressure_angle=pressure_angle);
                                                         // TODO: What is this parameter ???    ,addendum_adjustment=addendum_adjustment);
        // vertical hole for screwdriver inspection
        rotate([0, 90, 0])
            translate([8, 0, 0])
                  cylinder(r=bolt_size, h=40, $fs=circle_precision);   //center=true,
        // Motor mounts
        translate([0, 15, -20])
                cylinder(r=bolt_size, h=15, $fs=circle_precision);   //center=true,
        translate([0, -15, -20])
                cylinder(r=bolt_size, h=15, $fs=circle_precision);   //center=true,
    }
...
/**
 * bore_diam        Inner bore for motor shaft
 * gear_height      Thickness of the non geared part
 * gear_thickness   Thickness of the geared part
 */
module sun( pitch=0.865, teeth=8, gear_thickness=11, gear_height=8, bore_diam=5.5, , hub_diam=20, clearance=0.2, backlash=0.2, pressure_angle=pressure_angle, nut_size=6, nut_thickness=2.75, screw_diam=3) {
    difference() {
        translate([0,0,gear_height+gear_thickness])
            rotate([0,180,0])
            gear(number_of_teeth=teeth, diametral_pitch=pitch, hub_diameter=hub_diam, hub_thickness=gear_height+gear_thickness, bore_diameter=bore_diam,
                 rim_thickness=gear_thickness, rim_width=0, gear_thickness=gear_thickness,clearance=clearance, backlash=backlash, pressure_angle=pressure_angle);
        cylinder(r=bore_diam/2,h=gear_height+gear_thickness, $fs=circle_precision);        // cut out bore for motor shaft
        //cut hole for set screws
        translate([0,0,(nut_size*1.15)/2])
            rotate([0,90,0])
            cylinder(r=screw_diam/2,h=hub_diam+extra, center=true, $fs=circle_precision);
        // cut pocket for set screw nuts
        nut_size = nut_size+0.1;
        for (num=[0:1])
            rotate([0,0,180*num])
            translate([(-hub_diam+bore_diam)/2,-nut_size/2,0])
            cube(size=[nut_thickness,nut_size,nut_size*1.15]);
    }
}
/**/
